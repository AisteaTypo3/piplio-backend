...
`tx_pipliobackend_topic.color_key` is a fixed select with exactly these 35 values (they map to hardcoded icon/color themes in the app; an unmatched value still works but renders a generic fallback):

```
numbers20, addition20, subtraction20, numbers100, addition100, subtraction100,
make100, bridge100, times2_5_10, times3_4, times6_7_8_9, times_mixed, division_intro,
wall_math, clock, money, laengen, umrechnen_laenge, umrechnen_gewicht, umrechnen_volumen,
numbers1000, addition1000, subtraction1000, runden, write_digits, write_uppercase,
deutsch_artikel, deutsch_reime, deutsch_gross_klein, deutsch_wortarten, deutsch_plural,
deutsch_zeitformen, deutsch_rechtschreibung, deutsch_silben, deutsch_satzzeichen
```
...
- `title`
- `subtitle`
- `color_key` — fixed select, one of the 35 values listed under [Supported `colorKey` values](#supported-colorkey-values)
...
2. Set:
   ...
   - `Color Key` = pick from the fixed 35-value select (defaults to matching the topic id)
